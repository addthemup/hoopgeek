#!/usr/bin/env python3
"""
Get 2024-25 Season Stats for Current NBA Players
Imports the current season data
"""
...
def get_current_season_players() -> List[Dict[str, Any]]:
    ...
    print("🏀 Fetching 2024-25 season players...")
    
    try:
        from nba_api.stats.endpoints import commonallplayers
        
        # Get current season players (2024-25)
...
def get_player_season_stats(player_id: int, season: str = "2024-25") -> Optional[Dict[str, Any]]:
...
def main():
    """Main function"""
    print("🚀 Starting 2024-25 Season Stats Import")
    ...
    try:
        ...
        print("\n📋 STEP 1: Getting 2024-25 season players...")
        ...
        print(f"\n📊 STEP 2: Getting 2024-25 season stats for {len(current_players)} current players...")
        ...
        for i, player in enumerate(current_players):
            try:
                ...
                # Get season stats
                season_stats = get_player_season_stats(player_id, "2024-25")
                if season_stats:
                    ...
                    print(f"     ✅ Imported 2024-25 stats for {player_name} ({season_stats['gp']} games)")
                else:
                    print(f"     ⚠️  No 2024-25 season stats found for {player_name}")
        ...
        # Final results
        print("\n" + "=" * 80)
        print("🎉 2024-25 Season Stats Import Completed!")
        print(f"📊 Final Statistics:")
        print(f"   Current players processed: {len(current_players)}")
        print(f"   2024-25 season stats imported: {season_stats_imported}")
        ...
        # Check what seasons we have now
        seasons = supabase.table('player_season_stats').select('season').execute()
        unique_seasons = list(set([stat['season'] for stat in seasons.data]))
        
        print(f"✅ Total players in database: {players_count}")
        print(f"✅ Total season stats in database: {season_stats_count}")
        print(f"📊 Seasons available: {unique_seasons}")
        
        # Show sample data
        print("\n📋 Sample of 2024-25 season stats:")
        sample = supabase.table('player_season_stats').select('nba_player_id, season, gp, pts_per_game, reb_per_game, ast_per_game').eq('season', '2024-25').limit(5).execute()
