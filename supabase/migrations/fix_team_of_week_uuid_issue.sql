-- Fix Team of the Week UUID vs INTEGER comparison issue
-- The nba_hoopshype_salaries.player_id is INTEGER (nba_player_id), not UUID
...
BEGIN
  ...
  player_performance AS (
    ...
    -- FIX: Join on nba_player_id (INTEGER) instead of id (UUID)
...
Fixed to join nba_hoopshype_salaries on nba_player_id (INTEGER) instead of id (UUID).';
