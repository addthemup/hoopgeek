-- =====================================================
-- CREATE TEAM FAVORITES TABLE AND FUNCTIONS
-- =====================================================
-- This migration creates the user_favorite_teams table
-- and all associated functions for managing team favorites
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.user_favorite_teams (
    ...
    team_id INTEGER NOT NULL, -- References nba_teams.team_id (not UUID, but integer team_id)
    ...
    FOREIGN KEY (team_id) REFERENCES public.nba_teams(team_id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_user_favorite_teams_team_id ON public.user_favorite_teams(team_id);

-- Composite index for user + team lookups
CREATE INDEX IF NOT EXISTS idx_user_favorite_teams_user_team ON public.user_favorite_teams(user_id, team_id);
...
DROP POLICY IF EXISTS "Users can view their own favorite teams" ON public.user_favorite_teams;
...
DROP POLICY IF EXISTS "Users can add favorite teams" ON public.user_favorite_teams;
...
DROP POLICY IF EXISTS "Users can update their favorite teams" ON public.user_favorite_teams;
...
DROP POLICY IF EXISTS "Users can delete their favorite teams" ON public.user_favorite_teams;
...
BEGIN
    ...
    SELECT
        uft.id,
        uft.team_id,
        nt.city || ' ' || nt.nickname AS team_name,
    ...
    INNER JOIN public.nba_teams nt ON uft.team_id = nt.team_id
...
COMMENT ON COLUMN public.user_favorite_teams.team_id IS 'NBA Team ID (integer) from nba_teams.team_id';
