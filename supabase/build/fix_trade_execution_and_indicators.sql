-- =====================================================
-- FIX TRADE EXECUTION AND INDICATORS
-- =====================================================
-- This script fixes two issues:
-- 1. Trade execution not moving players between teams
-- 2. DraftPicksCarousel not showing trade indicators
-- =====================================================

-- First, ensure trade tracking columns exist in fantasy_draft_order
...
ADD COLUMN IF NOT EXISTS current_team_id UUID REFERENCES fantasy_team_id,
...
-- Drop and recreate the accept_trade_offer function with proper player movement
...
BEGIN
  ...
  (
    ...
    'trade',
    'Traded players/picks to ' || (SELECT team_name FROM fantasy_teams WHERE id = trade_record.to_team_id),
    NOW(),
    auth.uid()
  ),
  (
    ...
    'trade',
    'Received players/picks from ' || (SELECT team_name FROM fantasy_teams WHERE id = trade_record.from_team_id),
    NOW(),
    auth.uid()
  );
...
-- Create a function to get team names for trade indicators
CREATE OR REPLACE FUNCTION get_team_name(team_id_param UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  RETURN COALESCE(team_name_val, 'Unknown Team');
END;
$$;

-- Grant execute permissions
GRANT EXECUTE ON FUNCTION get_team_name(UUID) TO authenticated;

DO $$
BEGIN
    RAISE NOTICE '✅ Fixed trade execution to properly move players between teams';
    RAISE NOTICE '✅ Added trade tracking columns to fantasy_draft_order';
    RAISE NOTICE '✅ Updated accept_trade_offer function with proper player movement';
    RAISE NOTICE '✅ Created get_team_name helper function';
    RAISE NOTICE '✅ Trade system is now fully functional';
END $$;
