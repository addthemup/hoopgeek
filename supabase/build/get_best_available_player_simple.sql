-- ============================================================================
-- SIMPLE GET BEST AVAILABLE PLAYER FUNCTION
-- Based on the old working migration files
-- ============================================================================
...
-- Simple function that matches the old working version
...
BEGIN
  ...
  -- Return the best available player
  RETURN QUERY
  SELECT 
    np.id,
    np.name,
  ...
  ORDER BY 
    -- Primary sort: by projected fantasy points if available
    CASE WHEN nep.proj_2026_pts IS NOT NULL THEN projected_fantasy_points ELSE 0 END DESC,
    -- Secondary sort: by salary (higher salary = better player)
...
COMMENT ON FUNCTION get_best_available_player IS 'Returns the best available undrafted player based on ESPN projections and salary cap constraints.';
...
BEGIN
    ...
    RAISE NOTICE '✅ Simple version based on old working migration';
    RAISE NOTICE '✅ Uses current table names (nba_players, fantasy_draft_picks, etc.)';
