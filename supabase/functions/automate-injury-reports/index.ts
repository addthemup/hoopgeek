/**
 * Automate injury_report feed posts per NBA game (injuries for both teams).
 * Defers if no qualifying injuries for the matchup. Uses injury_report_batch_done checkpoint.
 */
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.38.4'
import { isDateInEST, getTodayEST, utcToESTDate } from '../_shared/prop_prediction/nbaDateUtils.ts'
import { buildInjuryReportSnapshot } from '../_shared/injury_report/buildInjuryReportSnapshot.ts'
...
async function processGame(
  ...
  if (!force) {
    const { data: row } = await supabase
      ...
      .select('injury_report_batch_done')
      .eq('game_id', gameId)
      .maybeSingle()
    if (row?.injury_report_batch_done === true) {
      return { game_id: gameId, skipped: true, reason: 'injury_report_batch_done' }
    }
  }

  const built = await buildInjuryReportSnapshot(supabase, {
    targetDate,
    targetTeams: [away, home] as [string, string],
  })
  ...
  if (!built) {
    return {
      game_id: gameId,
      deferred: true,
      reason: 'no_injuries_for_matchup',
      message: 'No qualifying injuries for these teams — will not create an empty post.',
    }
  }

  const title = `Injury Report — ${matchup} — ${targetDate}`
  const description = `Daily injury updates for ${cleanGameDate || targetDate} — who's out, questionable, or returning for ${matchup}.`
  ...
  const postRow = {
    ...
    person_id: null,
    tags: ['injuries'],
    metadata: { injury_snapshot: built.injury_snapshot },
  ...
  if (existingCp) {
    await supabase.from('feed_automation_checkpoints').update({ injury_report_batch_done: true }).eq('game_id', gameId)
  } else {
    await supabase.from('feed_automation_checkpoints').insert({
      game_id: gameId,
      injury_report_batch_done: true,
    })
  }
  ...
  return {
    ...
    source_ref: sourceRef,
    slug,
    injury_count: Array.isArray((built.injury_snapshot as { injuries?: unknown }).injuries)
      ? (built.injury_snapshot as { injuries: unknown[] }).injuries.length
      : 0,
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!targetDate) targetDate = getTodayEST()
