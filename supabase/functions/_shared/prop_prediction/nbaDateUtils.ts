...
/**
 * Previous calendar day in America/New_York, as YYYY-MM-DD (for morning automations that
 * target the prior night's slate).
 */
export function getYesterdayEST(): string {
  const [y, m, d] = getTodayEST().split('-').map(Number)
