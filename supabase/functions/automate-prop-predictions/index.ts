/**
 * Automate prop_prediction feed posts per NBA game for a slate date.
 * Queries player_props / player_props_games (same pipeline as PostCreator prop prediction generator).
 * Skips until props exist (no empty posts). Uses feed_automation_checkpoints.prop_prediction_batch_done to avoid duplicates.
 */
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.38.4'
import { isDateInEST, getTodayEST, utcToESTDate } from '../_shared/prop_prediction/nbaDateUtils.ts'
import { buildPropPredictionSnapshot } from '../_shared/prop_prediction/buildPropPredictionSnapshot.ts'
...
async function processGame(
  ...
  if (!force) {
    const { data: row } = await supabase
      ...
      .select('prop_prediction_batch_done')
      .eq('game_id', gameId)
      .maybeSingle()
    if (row?.prop_prediction_batch_done === true) {
      return { game_id: gameId, skipped: true, reason: 'prop_prediction_batch_done' }
    }
  }

  const built = await buildPropPredictionSnapshot(supabase, {
    targetDate,
    targetGameId: gameId,
    awayTricode: away,
    homeTricode: home,
  ...
  if (!built) {
    return {
      game_id: gameId,
      deferred: true,
      reason: 'no_prop_data_yet',
      message: 'Player props not ready or no full-game lines for this matchup — will not create an empty post.',
    }
  }
  ...
  if (existingCp) {
    await supabase.from('feed_automation_checkpoints').update({ prop_prediction_batch_done: true }).eq('game_id', gameId)
  } else {
    await supabase.from('feed_automation_checkpoints').insert({
      game_id: gameId,
      prop_prediction_batch_done: true,
    })
  }
  ...
  try {
    ...
    if (req.method === 'GET') {
      return new Response(
        JSON.stringify({
          message:
            'POST JSON: {"date":"2026-03-21"} processes all games that slate (EST calendar date). Optional: "game_id":"0022501028", "force":true. Date defaults to today (America/New_York).',
        }),
