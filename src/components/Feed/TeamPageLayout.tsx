/**
 * Team page layout using the same MUI Joy inset drawer pattern as /feed/ and /player/.
 *
 * Main area: team header + team posts feed. A button top-right opens an
 * inset drawer containing all team modules (player dashboard, rebounding,
 * shot dashboard, game logs, four factors). Which modules appear in the
 * drawer is controlled by /admin?view=team (Team Page Modules).
 */
...
interface TeamPageLayoutProps {
  ...
  drawerModules: TeamDrawerModule[];
  drawerHeaderContent?: React.ReactNode;
}

const MODULE_LABELS: Record<string, string> = {
  player_dashboard: 'Player Dashboard',
  rebounding: 'Rebounding Profile',
  shot_dashboard: 'Shot Dashboard',
  game_logs: 'Game Logs',
...
export default function TeamPageLayout({
  ...
  const { data: moduleVisibility } = useTeamModuleVisibility();
  const effectiveVisibility = moduleVisibility ?? DEFAULT_TEAM_MODULES;
  ...
  const modulesToRender = visibleModuleNames
    ...
    .filter(Boolean) as TeamDrawerModule[];
  ...
  const insetDrawer = (
    ...
    >
      ...
      >
        {!drawerHeaderContent && <DialogTitle>More</DialogTitle>}
        <ModalClose />
        {drawerHeaderContent && (
          <>
