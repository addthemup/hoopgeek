...
} from '../hooks/useNewRosterManagement';

interface NewTeamRosterProps {
  leagueId: string;
  teamId?: string;
}

export default function NewTeamRoster({ leagueId, teamId }: NewTeamRosterProps) {
  const { data: teams } = useTeams(leagueId);
  const { data: league } = useLeague(leagueId);
  const { data: rosterConfig } = useRosterConfig(leagueId);
  ...
  const { data: rosterSpots, isLoading, error } = useTeamRosterSpots(selectedTeam?.id || '');
  ...
  const getPositionLabel = (position: string, positionNumber: number) => {
    if (position === 'UTIL') {
      return `UTIL ${positionNumber}`;
    }
    if (position === 'BENCH') {
      return `BENCH ${positionNumber}`;
    }
    if (position === 'IR') {
      return `IR ${positionNumber}`;
    }
    if (position === 'TAXI') {
      return `TAXI ${positionNumber}`;
    }
    return position;
  };
  ...
  const calculateRosterProgress = () => {
    if (!rosterConfig || !rosterSpots) return {};

    const progress: Record<string, { filled: number; required: number }> = {};

    // Count filled spots by position
    rosterSpots.forEach(spot => {
      if (spot.player_id) {
        const key = spot.position;
        if (!progress[key]) {
          progress[key] = { filled: 0, required: 0 };
        }
        progress[key].filled++;
      }
    });

    // Set required counts from config
    if (rosterConfig.pg_count > 0) progress.PG = { ...progress.PG, required: rosterConfig.pg_count };
    if (rosterConfig.sg_count > 0) progress.SG = { ...progress.SG, required: rosterConfig.sg_count };
    if (rosterConfig.sf_count > 0) progress.SF = { ...progress.SF, required: rosterConfig.sf_count };
    if (rosterConfig.pf_count > 0) progress.PF = { ...progress.PF, required: rosterConfig.pf_count };
    if (rosterConfig.c_count > 0) progress.C = { ...progress.C, required: rosterConfig.c_count };
    if (rosterConfig.g_count > 0) progress.G = { ...progress.G, required: rosterConfig.g_count };
    if (rosterConfig.f_count > 0) progress.F = { ...progress.F, required: rosterConfig.f_count };
    if (rosterConfig.util_count > 0) progress.UTIL = { ...progress.UTIL, required: rosterConfig.util_count };
    if (rosterConfig.bench_count > 0) progress.BENCH = { ...progress.BENCH, required: rosterConfig.bench_count };
    if (rosterConfig.ir_count > 0) progress.IR = { ...progress.IR, required: rosterConfig.ir_count };
    if (rosterConfig.taxi_count > 0) progress.TAXI = { ...progress.TAXI, required: rosterConfig.taxi_count };

    return progress;
  };

  const rosterProgress = calculateRosterProgress();
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Card variant="outlined">
        <CardContent>
          ...
          {Object.keys(rosterProgress).length > 0 && (
            <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                {Object.entries(rosterProgress).map(([position, progress]) => {
                  const isComplete = progress.filled >= progress.required;
                  const positionName = position === 'G' ? 'Guard' : 
                                     position === 'F' ? 'Forward' : 
                                     position === 'C' ? 'Center' : 
                                     position === 'UTIL' ? 'Utility' : 
                                     position === 'BENCH' ? 'Bench' :
                                     position === 'IR' ? 'IR' :
                                     position === 'TAXI' ? 'Taxi' : position;
                  ...
                  return (
                    <Chip
                      ...
                      sx={{ fontWeight: 'bold' }}
                    >
                      {positionName}: {progress.filled}/{progress.required}
                    </Chip>
                  );
                })}
              </Stack>
                  ...
                  return (
                    ...
                    >
                      <td>
                        <Chip 
                          size="sm" 
                          color={getPositionColor(rosterSpot.position)} 
                          variant="soft"
                        >
                          {getPositionLabel(rosterSpot.position, rosterSpot.position_number)}
                        </Chip>
                      </td>
                      <td>
            ...
            <CardContent>
              ...
              <Stack spacing={1}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Typography level="body-sm" sx={{ fontWeight: 'bold' }}>
                    {rosterSpots?.filter(spot => spot.player_id).length || 0} / {rosterSpots?.length || 0}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography level="body-sm">Starters:</Typography>
                  <Typography level="body-sm" sx={{ fontWeight: 'bold' }}>
                    {rosterSpots?.filter(spot => spot.is_starter && spot.player_id).length || 0} / {rosterConfig?.max_starters || 10}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography level="body-sm">Bench:</Typography>
                  <Typography level="body-sm" sx={{ fontWeight: 'bold' }}>
                    {rosterSpots?.filter(spot => spot.is_bench && spot.player_id).length || 0} / {rosterConfig?.max_bench || 3}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography level="body-sm">IR:</Typography>
                  <Typography level="body-sm" sx={{ fontWeight: 'bold' }}>
                    {rosterSpots?.filter(spot => spot.is_injured_reserve && spot.player_id).length || 0} / {rosterConfig?.max_ir || 1}
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography level="body-sm">Taxi:</Typography>
                  <Typography level="body-sm" sx={{ fontWeight: 'bold' }}>
                    {rosterSpots?.filter(spot => spot.is_taxi_squad && spot.player_id).length || 0} / {rosterConfig?.max_taxi || 0}
                  </Typography>
                </Box>
              </Stack>
            </CardContent>
      ...
      {/* @ts-ignore - TypeScript type mismatch due to Player interface changes */}
      <BasketballCourtMatchup
        homeTeam={{
          name: selectedTeam.team_name,
          abbreviation: selectedTeam.team_name.substring(0, 3).toUpperCase(),
          starters: rosterSpots?.filter(r => r.is_starter && r.player).map(r => ({
          ...
          bench: rosterSpots?.filter(r => !r.is_starter && r.player).map(r => ({
            id: r.player!.id as string,
            name: r.player!.name,
            position: r.player!.position || 'N/A',
            jersey_number: r.player!.jersey_number,
...
// Enhanced Player Detail Component (same as before)
...
// Upcoming Games Tab Component (same as before)
