...
export default function AdminPlayer() {
  ...
  const { data: modules, isLoading: modulesLoading, error: modulesError } = useQuery<PlayerModuleVisibilityRow[]>({
    queryKey: ['player-module-visibility'],
    queryFn: async () => {
      ...
      return (data ?? []) as PlayerModuleVisibilityRow[];
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['player-module-visibility'] });
      queryClient.invalidateQueries({ queryKey: ['player-module-visibility-map'] });
    },
  });
  ...
  const displayModules = (modules ?? []).length > 0
    ? modules!
    : PLAYER_MODULE_DEFINITIONS.map((def, index) => ({
        id: undefined,
        module_name: def.id,
        is_visible: true,
        display_order: index,
      })) as PlayerModuleVisibilityRow[];
  ...
  const handleSave = async () => {
    ...
    if (error) {
      console.error('Save player module visibility error:', error);
  ...
  function SortableRow({ moduleName }: { moduleName: string }) {
    const def = PLAYER_MODULE_DEFINITIONS.find((d) => d.id === moduleName);
      ...
      <Card variant="outlined" sx={{ bgcolor: '#1a1a1a', borderColor: '#333333', height: '100%' }}>
        <CardContent>
          {modulesError && (
            <Alert color="warning" sx={{ mb: 2, bgcolor: '#3a2a1a', borderColor: '#5a4a2a' }}>
              <Typography sx={{ color: '#FFFFFF' }}>
                player_module_visibility table may not exist. Run migration 20260224200000_player_module_visibility.sql
              </Typography>
            </Alert>
          )}
          ...
          <Typography level="body-sm" sx={{ color: '#B0B0B0', mb: 3 }}>
            Choose which modules appear in the drawer on each player page (/player/:id). Drag to reorder. The player page main area shows a feed of posts for the player; the drawer holds these modules.
          </Typography>
          ...
          <Box sx={{ mt: 3 }}>
            ...
            >
              <Typography level="title-sm" sx={{ fontWeight: 'bold', mb: 1.5, color: 'text.primary' }}>Player Drawer</Typography>
              <Box
                sx={{
                  display: 'grid',
              ...
              >
                {moduleOrder
                  ...
                  .map((name) => {
                    const def = PLAYER_MODULE_DEFINITIONS.find((d) => d.id === name);
                    return (
                      <Card
                        key={name}
                        variant="outlined"
          ...
          {updateMutation.isSuccess && (
            <Alert color="success" sx={{ mt: 2, bgcolor: '#1a3a1a', borderColor: '#2d5a2d' }}>
              <Typography sx={{ color: '#FFFFFF' }}>Player module settings saved.</Typography>
            </Alert>
          )}
        </CardContent>
      </Card>
